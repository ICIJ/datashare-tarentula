...
class TestDatashareClient(TestCase):
    ...
    def test_document_creation(self):
        with self.datashare_client.temporary_project() as project:
            id = self.datashare_client.index(index = project,  document = { '_id': uuid.uuid4(), 'foo': 'bar' })
    ...
    def test_document_deletion(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { '_id': uuid.uuid4(), 'foo': 'bar' }, id = 'to-delete')
    ...
    def test_query_with_two_docs(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { '_id': uuid.uuid4(), 'name': 'Atypidae' })
            self.datashare_client.index(index = project,  document = { '_id': uuid.uuid4(), 'name': 'Migidae' })
    ...
    def test_query_with_three_docs(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { '_id': uuid.uuid4(), 'name': 'Atypidae' })
            self.datashare_client.index(index = project,  document = { '_id': uuid.uuid4(), 'name': 'Euctenizidae' })
            self.datashare_client.index(index = project,  document = { '_id': uuid.uuid4(), 'name': 'Migidae' })
            total = self.datashare_client.query(index = project).get('hits', {}).get('total')
    ...
    def test_get_document(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { '_id': uuid.uuid4(), 'name': 'Atypidae' }, id = 'atypidae')
