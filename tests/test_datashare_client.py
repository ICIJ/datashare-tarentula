...
def newDocumentId():
    documnet_id_seq = documnet_id_seq + 1
    return documnet_id_seq
...
class TestDatashareClient(TestCase):
    ...
    def test_document_creation(self):
        with self.datashare_client.temporary_project() as project:
            id = self.datashare_client.index(index = project,  document = { '_id': newDocumentId(), 'foo': 'bar' })
    ...
    def test_document_deletion(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { '_id': newDocumentId(), 'foo': 'bar' }, id = 'to-delete')
    ...
    def test_query_with_two_docs(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { '_id': newDocumentId(), 'name': 'Atypidae' })
            self.datashare_client.index(index = project,  document = { '_id': newDocumentId(), 'name': 'Migidae' })
    ...
    def test_query_with_three_docs(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { '_id': newDocumentId(), 'name': 'Atypidae' })
            self.datashare_client.index(index = project,  document = { '_id': newDocumentId(), 'name': 'Euctenizidae' })
            self.datashare_client.index(index = project,  document = { '_id': newDocumentId(), 'name': 'Migidae' })
            total = self.datashare_client.query(index = project).get('hits', {}).get('total')
    ...
    def test_get_document(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { '_id': newDocumentId(), 'name': 'Atypidae' }, id = 'atypidae')
