...
class TestDatashareClient(TestCase):
    ...
    def test_document_creation(self):
        with self.datashare_client.temporary_project() as project:
            id = self.datashare_client.index(index = project,  document = { 'foo': 'bar' })
    ...
    def test_query_with_two_docs(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { 'name': 'Atypidae' })
    ...
    def test_query_with_three_docs(self):
        with self.datashare_client.temporary_project() as project:
            self.datashare_client.index(index = project,  document = { 'name': 'Atypidae' })
            self.datashare_client.index(index = project,  document = { 'name': 'Euctenizidae' })
            self.datashare_client.index(index = project,  document = { 'name': 'Migidae' })
            total = self.datashare_client.query(index = project).get('hits', {}).get('total')
